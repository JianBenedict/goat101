...
class Employee{
	...
	public function new_employee($firstname,$lastname,$contactnumber,$email,$address,$type){
		...
		$data = [
			[$firstname,$lastname,$contactnumber,$email,$address,$type],
		];
		$stmt = $this->conn->prepare("INSERT INTO employee (firstname,lastname,contactnumber,email,address,type) VALUES (?,?,?,?,?,?)");
		try {
	...
	public function update_employee($firstname,$lastname,$contactnumber,$email,$id,$address,$type){
		
		$sql = "UPDATE employee SET firstname=:firstname,lastname=:lastname,contactnumber=:contactnumber,email=:email,EmployeeId=:EmployeeId,address=:address,type=:type WHERE EmployeeId=:EmployeeId";

		$q = $this->conn->prepare($sql);
		$q->execute(array(':firstname'=>$firstname,':lastname'=>$lastname,':contactnumber'=>$contactnumber,':email'=>$email,':EmployeeId'=>$id,':address'=>$address,':type'=>$type,));
		return true;
	}
...
public function delete_employee($id)
	...
	public function list_firstname_search($keyword){

		//$keyword = "%".$keyword."%";

		$q = $this->conn->prepare('SELECT * FROM `employee` WHERE (`firstname`) LIKE ?');
